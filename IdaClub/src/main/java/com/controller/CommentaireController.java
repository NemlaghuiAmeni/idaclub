...
public class CommentaireController {
	...
	@PostMapping("/comm/{id}/newRep")
	public ResponseEntity<Void> ajoutReponseToComm(@PathVariable int id, @RequestBody Reponse newRep) {

		comService.addReptoComment(id, newRep);
	...
	@DeleteMapping("/commentaire/{idC}/reponse/{id}")
	public ResponseEntity<Void> deleteSUser(@PathVariable long idC, @PathVariable long id) {
		commentaire c1	 = comService.deletecommentReponse(idC, id);
		if (c1 != null) {
			return ResponseEntity.noContent().build();
